...
logger = logging.getLogger('AToFACDN')
...
reply = "You've got some links to FurAffinity images, Here are the direct links to those images!\n\n" # Base reply
...
for comment in subreddit.stream.comments():
    ...
    splitcmt = comment.body.replace("full","view")
    splitcmt = re.findall(r'(furaffinity.net/view/\S+)', splitcmt)
    if allowedToReply(comment):
        for part in splitcmt:
            ...
            source = sourceExist(FACDNLink, comment.body)
            if not source:
                logger.info("Adding {0} to ALinks and {1} to FLinks".format(FALink, FACDNLink))
        ...
        for x in range(0, len(FALinks)):
            reply += "[Link]({0}) | Image Name: {1} | Artist: {2} | [Imgur Mirror]({3})\n\n ^Keywords: ".format(CDNLinks[x], name[x], artist[x], imgurMirror.uploadImage(CDNLinks[x],name[x]))
            if len(keywords[x]) == 0:
                reply += "^None"
            else:
                for y in keywords[x]:
